...
var GameLayer = (function(){
	...
	var clear = function() {
		...
		brickPadding = 16;
		labelPadding = 60;
		x1 = Game.contentX + wallThickness;
		x2 = x1 + Game.contentWidth - wallThickness * 2;
		y1 = Game.contentY + wallThickness;
		y2 = y1 + Game.contentHeight - wallThickness * 2;
		bg = null;
		bg2 = null;
		paddle = null;
		ball = null;
	...
	var createBrickExplosion = function(self, x, y) {
		...
		for (i = 0; i < 15; i += 1) {
			t = (250 + Game.rand(250)) / 1000;
			sprite = cc.Sprite.create("Particle.png");
			sprite.setPosition(x, y);
			sprite.runAction(cc.Sequence.create(
				cc.Spawn.create(
					cc.FadeOut.create(t),
					cc.EaseOut.create(cc.MoveBy.create(t, cc.p(xMin + Game.rand(xRange), yMin + Game.rand(yRange))), 1.2)
				),
	...
	return cc.Layer.extend({
		init: function() {
			...
			// Actual background
			bg = cc.LayerColor.create(cc.color(218, 214, 212, 255), Game.contentWidth, Game.contentHeight);
			bg.attr({x: Game.contentX, y: Game.contentY});
			this.addChild(bg, 0);
			
			// Labels
			scoreLabel = cc.LabelTTF.create("Score: " + Game.score.toString(), Game.config["font"], 45);
			...
			livesLabel = cc.LabelTTF.create("Lives: " + Game.lives.toString(), Game.config["font"], 45);
			...
			for (y = 0; y < numOfRows; y += 1) {
				for (x = 0; x < numOfCols; x += 1) {
					...
					i.x = Game.winSize.width * .5 + ((x + .5 + numOfCols * -.5) * w);
					i.y = Game.winSize.height * .5 + ((y + numOfRows * -.5) * h) + 40;
					i.body.setPos(cp.v(
						i.x,
						i.y
					));
				...
				postStepCallbacks.push(function(){
					for (var i = 0; i < numOfRows * numOfCols; i += 1) {
						if (bricks[i] && bricks[i].body === shape.body) {
							...
							brickCount -= 1;
							Game.score += 10;
							scoreLabel.setString("Score: " + Game.score);
							Game.playEffect("Brick.mp3");
							break;
						}
					}
			...
			space.addCollisionHandler(collisionTypeBall, collisionTypePaddle, null, null, null, function(arbiter, space){
				...
				Game.playEffect("Paddle.mp3");
			});
			space.addCollisionHandler(collisionTypeBall, collisionTypeBottom, function(arbiter, space){
				ball.body.setVel(cp.v(0,0));
				ball.body.setPos(cp.v(paddle.x, paddle.y + 50));
				Game.lives -= 1;
				livesLabel.setString("Lives: " + Game.lives);
				if (Game.lives <= 0) {
					var scene = new MenuScene;
					scene.init();
					cc.director.runScene(scene);
				} else {
					Game.playEffect("Die.mp3");
				}
				return false;
			}, null, null, null);
		...
		onExit: function() {
			...
			space.removeCollisionHandler(collisionTypeBall, collisionTypeBrick);
			space.removeCollisionHandler(collisionTypeBall, collisionTypePaddle);
			space.removeCollisionHandler(collisionTypeBall, collisionTypeBottom);
			space.removeCollisionHandler(collisionTypeBall, 0);
		}
